...
- **Leg-aware visibility is mandatory.** Reviews stay keyed per `alert_id`; there is no group-level review label; grouped views expose leg IDs by default and every leg row with `--expand`; groups whose siblings may be truncated by `--since`/`--limit` fetch bounds are flagged `partial_group=true` (never shown as complete) and excluded from reduction counts.
...
Validated offline against the frozen 44-alert labeled cohort (`reports/alert-quality/validate_cofire.py`): `removed_tp = 0`, `tp_leg_visible = 16/16`, `missed_labeled_hedge_fp = 0`; operator queue 44 → 22; FP operator items 13 → 5. Later larger-cohort revalidation is `tp=22` / `tp_leg_visible=22/22` after the v1.2 post-close corrections.

## Consequences

- The operator queue for hedged baskets roughly halves with **zero TP leg-visibility loss in the validated cohorts**. This is a leg-retention claim, not semantic proof that every retained tp label is correct. Governance (`alerts fp-rate`) is unchanged because reviews remain per-leg; it additionally attributes each breach's `cross_market_hedge` share (M-COFIRE-GOV).
...
- **"Grouped views always expose every leg" (Decision) is precise only with `--expand`** (default output = group summaries + leg IDs + partial indicators). A `review-packet --group-cofire` zero-reduction bug (a mandatory-default `review_state` forcing `filter_boundary`) was found by live run and **fixed in #98**.
