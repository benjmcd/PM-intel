...
def _dsn() -> str:
    if _SCRATCH_DB is None:
        raise RuntimeError("polymarket ingest scratch DB was not initialized")
    return _SCRATCH_DB.dsn


@pytest.fixture(scope="module", autouse=True)
def _polymarket_ingest_scratch_database():
    global _SCRATCH_DB  # noqa: PLW0603
    _SCRATCH_DB = create_test_scratch_database("polymarket_ingest")
    try:
        yield
    finally:
...
def test_polymarket_ingest_uses_scratch_db_not_configured_primary() -> None:
    assert _SCRATCH_DB is not None
    assert _dsn() != os.environ["PMFI_DB_URL"]
    assert _SCRATCH_DB.name.startswith(f"{TESTISO_DB_PREFIX}polymarket_ingest_")
