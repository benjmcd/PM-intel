...
>
> **Supersedes** `plans/2026-07-05-state-and-roadmap.md` (which remains valid history through PR #87 and the co-fire design debate; its inline "Live status" line is intentionally frozen at the co-fire-program-complete anchor #92 and should not be chased per-commit — this file is the live pointer).
>
> **Provenance.** Grounded in the repo at `origin/main = b5f7e2f` and the full 2026-07-06 working session: PRs #88–#96, three session-log reviews, multiple Opus verification workflows, an 318-alert population analysis, an operator-authorized live settlement re-label, and two orthogonal Codex audits (`M-AUDIT-CODE-INTEGRITY` / `M-AUDIT-RECORD-COMPLETENESS`, findings folded in §12). Every load-bearing claim cites a file:line, commit, PR#, or measured value. Corrections made after verification are shown inline (verify-don't-trust — the record shows its own error-corrections rather than hiding them; see the #94 overclaim in §2.3).

---

## 0. TL;DR — current state at a glance

- `origin/main = b5f7e2f`; suite `1393 passed / 94 skipped / 0 failed`; **0 open PRs**; 3 git worktrees (root + `prod-grade` unmerged + `v80` scratch).
- **The co-fire alert-quality program is feature-complete, durable, and honestly scoped across #88–#95.** It groups Kalshi multi-leg event co-fires so cross-market-hedge false positives are visible at review time. It is **read-side only and default-OFF**; live emission is byte-identical; governance stays per-alert.
- **A larger-cohort re-validation (#94/#95) confirmed the constants hold at population scale and produced a corrected, honest conclusion** (a prior "zero-tp-loss guarantee at scale" claim was caught as a Candidate-B tautology by dual adversarial review and reworded before merge). It also surfaced and fixed one real labeling bug (settlement-TP temporal guard, #95) and recorded one open known-limit (strike-ladder hedge modeling).
- **Nothing is blocking, but "no-risk toggle" was an over-assumption (corrected in §14).** Flipping `alerts list --group-cofire` on is low-risk (read-side, reversible, emission byte-identical) but **NOT verified-safe until the view is run end-to-end** — and a live run (§14) found that `review-packet --group-cofire` reports **reduction=0 unconditionally** (a real bug) and that **no CLI surface exposes a non-zero co-fire reduction count**. So the "flip it on" step is now **gated on running/verifying the view first**. Everything else forward is explicitly gated, dormant, or a deliberate non-goal (§5).
...
- **C — larger labeled gate (operator-authorized opt-in live settlement fetch, #94 `ef15cfb`).** Enlarged the labeled cohort to **110** (44 ∪ 66 new, deduped, existing labels win) by live-fetching Kalshi/Polymarket settlement for the new events and labeling under the same LABELING_RULE. Gate @900s PASS: `removed_tp=0`, `tp_visible=24/24`, `missed_labeled_hedge_fp=0`, `parent_label_conflicts=7`.

**The lesson (why the record is trustworthy).** The first C report claimed a "zero-tp-loss guarantee preserved at scale." Two independent adversarial reviews — an Opus verification workflow and Codex agent2 (a different model) — **both** caught that `removed_tp=0`/`tp_visible=24/24` are **unfailable identities under Candidate-B drop-nothing** (every leg is always in the group index → these checks cannot fail for any cohort). The genuinely load-bearing result is (b) `parent_label_conflicts=7`, which shows real mixed-label groups now exist (a BTCD group holds 3 tp legs with 1 fp leg) — this **strengthens** the ADR-0009 leg-aware-review mandate, it does NOT prove parent-level trust. (Note per §13: `missed_labeled_hedge_fp=0` is a derive/bucketing *consistency* guard — 0-by-construction when the candidate and grouping windows match — NOT an independent grouping discriminator; the window-sensitive 300s→FAIL is driven by the *hardcoded* GERCIV anchor-pair check in `validate_cofire.py`, not by `missed_labeled_hedge_fp`.) The report was corrected before merge; the overclaim never landed.
...
- **Temporal-guard fix (#95 `5f7db80`).** `fetch_outcomes.py` `settled_within_7d` had no lower bound → a fire AFTER `close_time` could be labeled tp via settlement (`9934a6e1` fired ~3.5h post-close). Fix: require `timedelta(0) <= (close - fired) <= SETTLE_D`; LABELING_RULE v1.1→v1.2; +2 offline monkeypatched tests (reject post-close / accept pre-close); `9934a6e1` deterministically re-labeled tp→noise (rule-forced, no live re-fetch); gate re-run PASS `tp_visible=23/23`.
...
- `origin/main = b5f7e2f` (#96); suite `1393 passed / 94 skipped / 0 failed`; 0 open PRs.
...
- 3 worktrees: root (`main`), `worktrees/PM-intel-grade` (`prod-grade`, only NOT-MERGED branch, kept), `worktrees/v80` (detached scratch, kept per operator; its uncommitted `.gstack/` line is now redundant with main).
...
1. **Operator flips `--group-cofire` on** — *what:* enable the read-side grouped view for triage. *why:* collapses co-fire legs into groups at review time (the reduction shows as fewer rows in `alerts list`; note the explicit reduction *count* is broken/absent per §14). *gating:* **must run the view end-to-end read-only first** (done once in §14 — `alerts list --group-cofire` renders clean on real data; but `review-packet --group-cofire` has the reduction=0 bug to fix or accept). *acceptance:* no error on real DB; correct partial_group on `--since`/`--limit` boundaries; DB fingerprint unchanged. *owner:* operator (toggle) after Claude/Codex fix the review-packet reduction bug. *why-optimal:* read-side + reversible, but "verify-before-enable" replaces the earlier "zero-risk" framing (a prior pagination bug + this reduction bug both escaped review lanes and were only caught by execution).
...
- Labeler: `reports/alert-quality/fetch_outcomes.py` (LABELING_RULE v1.2; opt-in live; `settled_within_7d` now lower-bounded).
- Cohorts: `outcomes-2026-07-02.json` (44, tracked, immutable v1.1 snapshot), `outcomes-cofire-reval-2026-07-06.json` (110, `9934a6e1`=noise).
...
- Gates: `verify.py` 1393/94; `validate_cofire` 44-cohort PASS (16/16), enlarged cohort PASS (`tp_visible=23/23`); `db_local.py verify` PASS.
...
- **Stale v1.2 companion artifacts:** after #95's relabel, `co-fire-validation-reval-2026-07-06.md` (+ w300/w1800), `cofire-reval-new-labels-2026-07-06.md`, `outcomes-cofire-reval-new-2026-07-06.json`, and the enlarged JSON's top-level `"rule"` metadata still say v1.1 / tp=24 / `9934a6e1`=tp. After the `9a357683` fix the consistent v1.2 numbers are tp=22 — these must be regenerated or superseded-bannered.
- **Stale committed plan/spec:** `plans/2026-07-05-state-and-roadmap.md` still says main=#92, frames R3 as emission, and lists `parent_label_conflicts==0` as a HARD gate (validate_cofire treats it as diagnostic only). This file (2026-07-06) supersedes it, but the old one needs a supersession pointer (or the new one committed).
...
**OPERATOR DIRECTION (2026-07-06, ratified):** (1) **FINALIZE THE RECORD ONLY — execute NO fixes this session; nothing on main changes.** The items below are the agreed plan for a future fix session, not actions taken now. (2) **81-tp vs R3 (scope A): INVESTIGATE BEFORE DECIDING** — a read-only Opus analysis must first characterize the 81 tp against R3 + settlement and recommend *narrow-R3* (rule too broad) vs *relabel-81* (data wrong); do not touch labels/rule until that analysis lands. (3) **DB + 44-cohort (scope B): when fixing, CORRECT THE JSON COHORTS** (relabel `9a357683` tp→noise in the reval cohort → tp=22, and flag/decide the 44-cohort's frozen copies) **but HOLD the `alert_reviews` DB re-ratification** for explicit operator ratification; document the JSON-vs-DB divergence in the interim.

**FIX PLAN (agreed for a future fix session per the operator direction above; see §5 sequencing):**
1. Re-label `9a357683` tp→noise in the enlarged cohort (offline, rule-forced, like `9934a6e1`); re-run the gate (expect `tp_visible=22/22` PASS). Treat the 44-cohort JSON as the immutable v1.1 snapshot (documented) as was done for `9934a6e1`.
2. Regenerate or supersede-banner the stale v1.2 companion artifacts to the corrected tp=22.
3. Add the strike-ladder known-limit to `fetch_outcomes.py`; tighten the ADR wording; add a supersession pointer on the 07-05 plan (or commit this doc).
...
**Path to "entirely certain" (recommended verifications, read-only unless noted):**
- Run the grouped view end-to-end against the live DB (`--group-cofire`/`--expand`/`review-packet`, with/without `--since`/`--limit` near a co-fire boundary); confirm no error + correct partial_group/reduction. [closes #1]
- Reconcile the 81 multi-market-event `tp` against R3: decide genuine-tp vs survivorship-hedge-fp, or document why the outcome test overrides R3 for the ratified corpus. [closes #2 — governance decision]
- Re-derive both BREACH numbers with exact recorded denominators; replace the stale 31.5%. [closes #3]
- Make `_cofire_item` prefer `facts.event_ticker` (or add a cross-path equivalence test + doc). [closes #4]
- Decide `momentum_v1` (and any other cross_market_hedge rule) in-scope vs explicitly out-of-scope. [closes #5]
- Refresh the MEMORY/doc DB snapshot to the current state. [closes #7]
...
- **[HIGH] NEW BUG — `review-packet --group-cofire` reports `reduction=0` and marks EVERY group partial, unconditionally.** Root cause: `cmd_alerts_review_packet` (`alerts.py:1319-1326`) includes `review_state` in the `any([...])` that appends `filter_boundary`, but `review_state` defaults to `'reviewed'` (`:1244`, never falsy) → `filter_boundary` is always set → all groups partial → `non_partial_reduction=0` always. A unit-test-only pass never caught this; execution did.
- **[MEDIUM] No CLI surface ever exposes a non-zero co-fire reduction count.** The count is emitted in exactly one place (`alerts.py:1360`, review-packet JSON) which is always 0; `alerts list` has no reduction count at all. The 44→22 reduction was ever only observed in the offline `validate_cofire.py` harness — never in the product. (The *grouping* benefit — fewer rows — does render in `alerts list`; the explicit reduction *metric* is broken/absent.)
...
- **WORKLOG.md was not updated** for this audit/meta-verify/fresh-pass session (repo protocol requires it). A WORKLOG entry is added recording the `9a357683` discovery, the RESIDUAL_UNCERTAINTY verdict, the review-packet bug, and this fix-plan.
...
**Operator decision still required:** (a) authorize the `fetch_outcomes.py` R3 narrowing (v1.3) — and confirm whether any same-actor/account field even exists in the review packet to make the strongest hedge gate implementable; (b) adjudicate per-leg the small losing-leg subset (keep tp on independent magnitude/live-window evidence, or flip to fp as survivorship artifacts). Everything else — the 66 evidence-grounded tp, the KXBTCD directional, ADR-0009, live emission — stays untouched.
...
| 4 | Fix `review-packet --group-cofire` reduction=0 bug | Cx impl + Op-us verify | a red-first test fails on the current always-0 behavior; `review_state`'s mandatory default no longer forces `filter_boundary`; a live read-only run shows a non-zero `non_partial_reduction`; flag-off byte-identical | run-view-first (done §14) |
| 5 | Re-derive both BREACH denominators; replace stale 31.5% | Op-us/Cl analysis | each rule's not-actionable % reproducible on the live DB with the exact recorded denominator; 31.5% replaced or its "current-floor" denominator defined | none |
| 6 | Narrow R3 (v1.2→v1.3) in `fetch_outcomes.py` | Cx impl + Op-us verify | `elif r.get("hedge_group")` no longer short-circuits before the OT-TP test (becomes a caveat on an otherwise-evaluated leg); requires ≥2 opposite-side legs or size-symmetry before asserting `cross_market_hedge`; red-first test: a genuine in-side + kept-move co-firing leg is NOT auto-fp'd; docstring v1.3; touches only this report-dir script (no `src/`) | Op authorization (§15) |
