...
> **Purpose.** The single canonical, evidence-grounded, self-contained record of: (a) **what PMFI is** (domain + architecture, §0.5); (b) **what has been accomplished/verified** (§1); (c) the **current true state** (§2); (d) **loose ends** (§3); (e) **what we intend next and why each choice is the most adequate** given the constraints (§4); (f) the **orchestration/delegation model** (§5); (g) **operational/hardware/safety guardrails** (§6); (h) **acceptance-criteria conventions** (§7); (i) **open operator decisions** (§8); (j) the **decision ledger** with alternatives + justification (§10); (k) **glossary** (§11); (l) **risk register** (§12). This is the plan-of-record. `WORKLOG.md` = *verified code progress only*; `docs/` = durable architecture/ADRs; `MEMORY.md` = orchestrator cross-session pointer. A reader should need nothing outside this file + the repo to act.
>
> **Live status (2026-07-06):** `origin/main = b1bbaa8` (co-fire program + artifacts through PR #91); suite `1386 passed / 94 skipped / 0 failed`; 0 open PRs. **CO-FIRE PROGRAM COMPLETE — R0 (semantics) ✅ · R2 (category governance, #88) ✅ · R1 (offline viability, #89) ✅ · R3 (read-side grouped view, #90, default-OFF) ✅.** Read-side `--group-cofire`/`--expand` grouped view at `alerts list` + `review-packet`; live emission byte-identical; queue reduction (44→22) at the read layer, flag default-OFF (operator flips when ready). **Verified through the full redundancy chain:** agent1 build → agent2 adversarial review → agent1 harden (pagination/`<=`/hedge-graph/tp-visibility) → **Opus verify (caught a limit-boundary partial-detection bug both missed)** → agent1 fix → **Opus re-verify (formal soundness proof + mutation-kill, PASS)**. `origin/main = b1bbaa8` (through PR #91, co-fire artifacts tracked), suite 1386/94/0. Operator standing decisions: `.codex/config.toml` as-is; no worktree/venv cleanup (§8). Both Codex threads idle/standing-by.
>
...
- **What (lean, decisive).** (a) Build a **standalone co-fire primitive** `src/pmfi/pipeline/cofire.py` — pure functions: `derive_event_ticker(venue_market_id)` (Kalshi `rsplit("-",1)[0]`, == `facts.event_ticker`, guarded to `None` for non-3-segment/non-Kalshi) and `group_cofire(alerts, window_s=900)` (**pairwise-radius**, event_ticker-scoped, Kalshi-only, **Candidate B: co-present, drop nothing**, every leg's identity/label retained). **Not wired into any live path.** (b) A **validation harness** that applies `group_cofire` to the 44-label cohort (`outcomes-2026-07-02.json`) and computes the **leg-aware zero-tp-loss gate**: `removed_tp==0`, `tp_leg_visible_after==all`, `parent_label_conflicts==0`, `missed_labeled_hedge_fp==0` (@900 s radius), plus the **fp/queue-reduction** delta. (c) Report `reports/alert-quality/co-fire-validation-2026-07-XX.md` = the **go/no-go evidence for R3**.
...
- **Acceptance criteria (pass/fail).**
  ...
  - Validation on the 44 cohort: **`removed_tp==0` AND `parent_label_conflicts==0` AND `missed_labeled_hedge_fp==0`** → R3 viable; report the fp-reduction achieved.
...
|---|---|---|---|
| **R3 over-suppresses a real tp** (hidden under an fp parent) | med (if R3 built) | high (silent signal loss) | Hard leg-aware zero-tp-loss gate (`tp_leg_visible_after`, `parent_label_conflicts==0`) + offline replay + adversarial review before merge; R3 gated. |
